...
def get_decks(n_decks: int, seed: np.random.default_rng, 
...
def init_seed(filepath = "data"):
    ...
    new_seed_file = f"init_state_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.json"
...
def save_decks(n_decks: int, seed_path: str = None, filepath = "data") -> str:
