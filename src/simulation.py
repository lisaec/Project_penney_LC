...
def play_game(deck: np.ndarray, player_1_seq: list, player_2_seq: list) -> dict:
...
def simulate_combination(player_1_seq, player_2_seq, decks, old_wins = None) -> dict:
...
def simulate_games(deck_path: str, combinations: list = COMBINATIONS) -> tuple[pd.DataFrame, int, dict]:
    ...
    #saving results
    results_filename = f"data/results_{n_games}_decks_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.csv"
    results.to_csv(results_filename, index = False)

    #saving dict of wins
    wins_filename = f"data/wins_{n_games}_decks_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.json"

    with open(wins_filename, 'w') as f:
...
def process_results(results: pd.DataFrame, n_games: int) -> tuple[np.ndarray,np.ndarray,np.ndarray,np.ndarray]:
...
def add_decks (old_wins: dict, old_results: pd.DataFrame, new_deck_path: str, n_old_games: int, combinations: list = COMBINATIONS) -> tuple[pd.DataFrame, int, dict]:

    """adds additional simulations to existing simulations and returns the same thing
    as simulate games"""
    ...
    n_games = n_old_games + len(new_decks)
